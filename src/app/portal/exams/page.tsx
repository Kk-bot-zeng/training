          ...
          <List.Item>
            <Card hoverable={canAttempt} style={{ borderRadius: 16, opacity: canAttempt ? 1 : 0.72 }}
              ...
              <Button type="primary" block disabled={!canAttempt} style={{ marginTop: 12, borderRadius: 10 }}>
                {e.returnedForRetake ? "管理员已打回，请重新考试" : canAttempt ? ((e.completedAttempts as number) > 0 ? "再次练习" : "开始考试") : e.availability === "not_started" ? "考试尚未开始" : e.availability === "ended" ? "考试已结束" : "已完成，不可重复考试"}
              </Button>
            </Card>
          </List.Item>
        );
